...
def application_main(request):
    ...
    return_data = {'apps': apps, 'fault_detection_preinfo':fault_detection_preinfo(),
                   'thermostat_control_preinfo':thermostat_control_preinfo()}
...
def application_individual(request, app_agent_id):
    ...
    if app_type.app_name == 'iblc':
        ...
        return_data = thermostat_control_info(app_agent_id)
        return_data.update(get_device_list_and_count(request))
...
def fault_detection_preinfo():
    ...
    available_thermostats = DeviceMetadata.objects.filter(approval_status='APR', device_type_id=1).exclude(
        agent_id__in=used_thermostat_ids)

    return {'available_thermostats':available_thermostats}


def thermostat_control_preinfo():
    thermo_control_apps = ApplicationRunning.objects.filter(app_type__app_name='thermostat_control')
    used_thermostat_ids = []
...
def thermostat_control_info(app_agent_id):
    data = {}
